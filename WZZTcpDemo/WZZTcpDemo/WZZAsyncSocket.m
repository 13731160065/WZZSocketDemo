...
NSString *const WZZAsyncSocketException = @"AsyncSocketException";
NSString *const WZZAsyncSocketErrorDomain = @"AsyncSocketErrorDomain";
...
@interface WZZAsyncSocket (Private)
...
**/
@interface WZZAsyncReadPacket : NSObject
{
  @public
...
@implementation WZZAsyncReadPacket
...
**/
@interface WZZAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation WZZAsyncWritePacket
...
- (void)checkForThreadSafety
{
	if (theRunLoop && (theRunLoop != CFRunLoopGetCurrent()))
	{
		...
		[NSException raise:WZZAsyncSocketException
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[WZZAsyncReadPacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[WZZAsyncWritePacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
		if (total != NULL) *total = 0;
...
- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	else
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:WZZAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
		}
		return NO;
	}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		WZZAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		if(([theCurrentRead isKindOfClass:[WZZAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:WZZAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
}
...
- (NSError *)getAbortError
{
	...
	return [NSError errorWithDomain:WZZAsyncSocketErrorDomain code:AsyncSocketCanceledError userInfo:info];
}
...
- (NSError *)getConnectTimeoutError
{
	...
	return [NSError errorWithDomain:WZZAsyncSocketErrorDomain code:AsyncSocketConnectTimeoutError userInfo:info];
}
...
- (NSError *)getReadMaxedOutError
{
	...
	return [NSError errorWithDomain:WZZAsyncSocketErrorDomain code:AsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)getReadTimeoutError
{
	...
	return [NSError errorWithDomain:WZZAsyncSocketErrorDomain code:AsyncSocketReadTimeoutError userInfo:info];
}
...
- (NSError *)getWriteTimeoutError
{
	...
	return [NSError errorWithDomain:WZZAsyncSocketErrorDomain code:AsyncSocketWriteTimeoutError userInfo:info];
}
                        ...
                        tag:(long)tag
{
	...
	WZZAsyncReadPacket *packet = [[WZZAsyncReadPacket alloc] initWithData:buffer
	                                                    startOffset:offset
	                                                      maxLength:length
	                                                        timeout:timeout
	                                                     readLength:0
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	WZZAsyncWritePacket *packet = [[WZZAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	WZZAsyncSocket *theSocket = [[(WZZAsyncSocket *)pInfo retain] autorelease];
